...
async function runIntegrityAudit() {
  ...
  try {
    ...
    console.log(chalk.red("❌ L3: Orchestrator Link Failed. Check RENDER_URL."));
  }

  // 2. AUDITORÍA TÁCTICA (TURSO)
  try {
    const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/admin/status`, {
      headers: { Authorization: `Bearer ${process.env.WORKER_AUTH_TOKEN}` }
    });
    if (res.status === 200) {
      console.log(chalk.green("✅ L3: Tactical Vault is Accessible (Turso/libSQL)"));
      results.tactical_db = true;
    }
  } catch (e) {
    console.log(chalk.red("❌ L3: Tactical Auth Failed. Check WORKER_AUTH_TOKEN."));
  }
  ...
  try {
    ...
    const { error } = await supabase.from('archived_jobs').select('count', { count: 'exact', head: true });
    if (!error) {
      console.log(chalk.green("✅ L4: Strategic Archive is Secure (Supabase)"));
      results.strategic_db = true;
    }
  } catch (e) {
    console.log(chalk.red("❌ L4: Strategic Link Failed. Check SUPABASE_KEYS."));
  }
  ...
  try {
    ...
    if (res.status === 200) {
      console.log(chalk.green("✅ OPS: Miner Static Binary is Publicly Accessible"));
      results.binary_host = true;
    }
  } catch (e) {
    console.log(chalk.red("❌ OPS: Miner Binary 404. Check GITHUB_RELEASES_URL."));
  }
