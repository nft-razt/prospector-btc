...
async function deployWorkerWithRetry(
  ...
  try {
    ...
    if (msg === 'AUTH_REQUIRED') {
        console.error(chalk.bgRed.white(`${workerPrefix} 🛑 ABORTANDO POR FALLO DE AUTENTICACIÓN.`));
        return;
    }
...
async function main() {
  console.log(chalk.bold.green('⚡ PROSPECTOR PROVISIONER v4.5 (HYDRA SWARM)'));
  console.log(`🎯 Shard Offset: ${SHARD_OFFSET} | Count: ${WORKER_COUNT} | Headless: ${config.HEADLESS}`);
  ...
  try {
    ...
    if (identityEmail) {
        ...
        console.log(chalk.yellow('👤 Modo: ANÓNIMO / LOCAL (Sin persistencia en Vault)'));
    }
    ...
    setInterval(() => {
       ...
       console.log(`[${new Date().toISOString()}] Heartbeat | Heap: ${(mem.heapUsed/1024/1024).toFixed(2)} MB`);
    }, 1000 * 60 * 5);
