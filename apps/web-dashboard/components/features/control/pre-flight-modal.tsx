 ...
 */
export function PreFlightModal({ isOpen, onClose, onConfirm, config }: PreFlightModalProps) {
  ...
  const updateCheck = useCallback((id: string, status: CheckStatus, message?: string) => {
  ...
  const runDiagnostics = useCallback(async () => {
    ...
    try {
      ...
      if (status.nodeCount > 0) {
        updateCheck('vault', 'success', `${status.nodeCount} Identities Available`);
      } else {
        ...
        <div className="p-6">
                ...
                {checks.map((check) => (
                    <div
                        ...
                        className={cn(
                            ...
                            check.status === 'checking' ? "bg-zinc-900/50 border-zinc-700" :
                            check.status === 'success' ? "bg-emerald-950/10 border-emerald-900/30" :
                            check.status === 'failure' ? "bg-red-950/10 border-red-900/30" :
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className={cn(
                                ...
                                check.status === 'failure' ? "text-red-400" : "text-zinc-300"
                            )}>
                                {check.label}
                            </span>
                        </div>
                        ...
                        {check.message && (
                            <span className={cn(
                                ...
                                check.status === 'success' ? "text-emerald-500 bg-emerald-500/10" :
                                check.status === 'failure' ? "text-red-500 bg-red-500/10" :
                                "text-zinc-500"
                            )}>
                                {check.message}
                            </span>
            ...
            <div className="mb-6 p-4 bg-zinc-900/30 border border-zinc-800 rounded-lg flex items-center justify-between">
                <div className="flex flex-col">
                    <span className="text-[9px] text-zinc-500 uppercase font-bold mb-1">Target Capacity</span>
                    <div className="flex items-baseline gap-1">
                        <span className="text-2xl font-black text-white">{totalWorkers}</span>
                        <span className="text-[10px] text-zinc-600 font-mono">NODES</span>
                    </div>
                </div>
                <div className="h-8 w-px bg-zinc-800 mx-4" />
                <div className="flex flex-col items-end">
                    <span className="text-[9px] text-zinc-500 uppercase font-bold mb-1">Infrastructure</span>
...
function StatusIcon({ status }: { status: CheckStatus }) {
    switch (status) {
        ...
        default: return <div className="w-4 h-4 rounded-full border-2 border-zinc-800 border-t-zinc-600" />;
    }
}
