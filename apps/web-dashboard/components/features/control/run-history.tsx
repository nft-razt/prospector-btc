...
export function RunHistory() {
  const { data: runs, isLoading, isError, refetch } = useQuery<WorkflowRun[]>({
    ...
    return (
        <Card className="bg-[#0f0f0f] border-slate-800 h-full flex flex-col">
            ...
            <CardContent className="p-0">
                {[1, 2, 3].map(i => (
                    <div key={i} className="p-4 border-b border-white/5 flex justify-between">
  ...
  if (isError) {
      return (
        <Card className="bg-[#0f0f0f] border-red-900/30 h-full flex flex-col justify-center items-center p-6 text-center">
            ...
            <h3 className="text-xs font-bold text-red-400 uppercase tracking-widest">Signal Lost</h3>
            <p className="text-[10px] text-zinc-500 mt-1 mb-4">Cannot retrieve C2 telemetry.</p>
            <button
                onClick={() => refetch()}
...
function RunItem({ run }: { run: WorkflowRun }) {
    const getStatusConfig = () => {
        if (run.status === 'in_progress' || run.status === 'queued') {
            return { icon: PlayCircle, color: 'text-yellow-500', bg: 'bg-yellow-500/10', animate: true };
        }
        if (run.conclusion === 'success') {
            return { icon: CheckCircle2, color: 'text-emerald-500', bg: 'bg-emerald-500/10', animate: false };
        }
        if (run.conclusion === 'cancelled') {
            return { icon: StopCircle, color: 'text-zinc-500', bg: 'bg-zinc-500/10', animate: false };
        }
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col min-w-0">
                    <span className="text-xs font-bold text-zinc-200 truncate group-hover:text-white transition-colors">
                        {run.name} <span className="text-zinc-600 font-mono">#{run.run_number}</span>
                    </span>
                    <div className="flex items-center gap-2">
                        <span className={cn("text-[9px] uppercase font-bold tracking-wider", config.color)}>
