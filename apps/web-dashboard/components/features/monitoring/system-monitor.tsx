...
import {
  Activity, Cpu, Server, Key, Wifi, WifiOff,
  Database, Search, Skull, Zap, BarChart3, ShieldCheck,
  Target
} from "lucide-react";
...
export function SystemMonitor() {
  ...
  const { metrics, isConnected, isLoading: isL3Loading } = useRealTimeTelemetry();
  ...
  return (
    <div className="w-full space-y-12 animate-in fade-in duration-1000">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
          <div className="space-y-2 text-center md:text-left">
            ...
            <p className="text-2xl font-black text-white font-mono tracking-tighter">
              {globalEffort?.total_keys_audited || "0"} <span className="text-zinc-500 text-sm tracking-normal">Billions of Identites Scanned</span>
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label={t("analytics.total_effort")} value={`${globalEffort?.total_keys_audited || "0"} B`} icon={BarChart3} color="purple" loading={isGlobalLoading} />
        <StatCard label="Live Hashrate" value={metrics?.global_hashrate ? `${(metrics.global_hashrate / 1e3).toFixed(1)} kH/s` : "0"} icon={Activity} color="emerald" loading={isGlobalLoading} />
        <StatCard label="Swarm Nodes" value={metrics?.active_nodes || 0} icon={Server} color="blue" loading={isGlobalLoading} />
        <StatCard label="Active Targets" value={metrics?.jobs_in_flight || 0} icon={Key} color="amber" loading={isGlobalLoading} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-4 flex flex-col gap-6">
           <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 flex flex-col justify-center items-center text-center space-y-4">
              ...
              <h4 className="text-[10px] font-black text-white uppercase tracking-widest font-mono">Kernel V14.0 Active</h4>
              <p className="text-[9px] text-zinc-500 font-mono">Assembler-level arithmetic optimization enabled for all grid units.</p>
           </div>
        </div>
      </div>
      ...
      <div className="pt-6 border-t border-white/5 flex flex-col lg:flex-row justify-between items-center gap-6">
        ...
        <div className="text-[9px] font-bold text-zinc-800 font-mono uppercase tracking-tighter">
          Prospector.OS // Build_{new Date().getTime().toString(16).toUpperCase()}
        </div>
      </div>
    </div>
  );
