...
export const apiClient = {
    ...
    put: <T>(url: string, data?: any, conf?: any) => getClient().put<T>(url, data, conf),
...
export const adminApi = {
  ...
  checkIdentityStatus: async (): Promise<IdentityStatusResponse> => {
      try { return (await getClient().get<IdentityStatusResponse>('/admin/identities/status')).data; }
  ...
  broadcastCommand: async (command: 'shutdown' | 'restart') => getClient().post('/admin/command', { command })
};
